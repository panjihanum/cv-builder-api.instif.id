...
.hero { display: flex; align-items: center; gap: 18px; padding: 24px 28px; color: #ffffff; background: linear-gradient(120deg,#4338ca 0%,#7c3aed 45%,#db2777 100%); }
...
.body { display: grid; grid-template-columns: 1fr 36%; flex: 1; }
  ...
  const entries = data.experience
    .map((item) => {
      ...
      return `<div class="xp"><div class="top"><h3>${escapeHtml(
        item.position
      )}</h3>${pill}</div>${orgLine}${renderDescription(item.description)}</div>`;
    })
    .join("");
  return mainSection("experience", t.experience, entries);
}
...
function renderProjects(data: CvData): string {
  ...
  const entries = data.projects
    .map((project) => {
      ...
      return `<div class="blk"><h3>${heading}</h3>${renderDescription(
        project.description
      )}</div>`;
    })
    .join("");
...
function renderSkills(data: CvData): string {
  ...
  return sideSection("skills", t.skills, bars);
}
